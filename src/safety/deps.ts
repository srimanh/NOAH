 ...
 *
 * NOAH runs on a third-party agentic runtime, but it must NOT inherit that
 * runtime's release cadence or any compromise in one of its patch releases. We
 * pin the exact runtime version we have vetted and verify it — across the WHOLE
 * installed tree (including nested copies npm may resolve via `^` ranges).
 *
 * Combined with `bundleDependencies` (we ship the vetted runtime inside our
 * tarball), this means: even if a malicious upstream release lands on the
 * registry, a NOAH install neither downloads nor runs it.
 */
...
export function formatViolations(violations: DepViolation[]): string {
  if (violations.length === 0) return "✓ core dependencies verified — all pinned versions intact.";
  const lines = violations.map(
    (v) => `  ✗ ${v.name}: expected ${v.expected}, found ${v.found}\n      ${v.path}`,
  );
  return `Supply-chain integrity check FAILED — core dependency drift detected:\n${lines.join("\n")}`;
}
